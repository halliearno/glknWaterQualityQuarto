...
sites_to_get <- locations$MonitoringLocationIdentifier#[1:11] 
...
filtered = data.filter(function(waterFilter) {
  return yearsmin <= waterFilter.Year &&
         ...
         monthhmin <= waterFilter.Month &&
         monthmax >= waterFilter.Month &&
...
//should also make an "all" 
viewof site = Inputs.checkbox(
  sitechoices.map(d => d.Site), 
...
names = station.map(({glknid:actualValue})=>actualValue);
```
...
// Maximum month slider
viewof monthhmin = Inputs.range(
  [1, 12], 
  { value: 1, step: 1, label: "Min month:" }
)

// Maximum month slider
viewof monthmax = Inputs.range(
  [1, 12], 
  { value: 12, step: 1, label: "Max month:" }
)
