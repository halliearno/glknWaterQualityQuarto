...
```{ojs}
window.alert("This visualizer is in draft form. Please do not distribute. See email for password instructions.");
```
<!--head-->
...
# Add lat long
locations <- locations %>% select(GLKNStationID, LatitudeMeasure, LongitudeMeasure, StationName)
...
ojs_define(GLKNStation = transpose(locations))
```
...
lat = GLKNStation.map(({Latitude:actualValue})=>actualValue);  //need to turn data into array (list of numbers to display on y-axis)
      
long = GLKNStation.map(({Longitude:actualValue})=>actualValue); //list of dates for x axis
          
names = GLKNStation.map(({Name:actualValue})=>actualValue);
```
...
//Park selection
viewof park = Inputs.checkbox(
  ["SACN", "MISS", "ISRO", "VOYA", "SLBE", "INDU", "PIRO", "APIS"], 
  { value: ["SACN"], multiple: true, label: "Park:" }
)
...
//can change back to names here: https://observablehq.com/@observablehq/input-checkbox
//should also make an "all" option
viewof site = Inputs.checkbox(
  sitechoices.map(d => d.Name), 
