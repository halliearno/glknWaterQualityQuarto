...
locations <- read.csv("GLKNStation.csv") %>% # Site names and info for all parks 
 mutate(GLKNStationID = str_extract(MonitoringLocationIdentifier,"[^\\-]+$"))

#Extract sites from lake parks spreadsheet
sites_to_get <- locations$MonitoringLocationIdentifier[1:15]
...
toggleFeature = (e) => {
  ...
  set(site, results); //USED TO BE VIEWOF SITE!!!
...
markers = {
 ...
 var markers = {};
    ...
    for (var i = 0; i < 74; i++) { //need to replace 74 but putting a variable length here breaks it so I am afraid to. 
      ...
      id: list_of_pairs[i][0]})  //also this 
    //Add event listeners 
...
//Figure out which sites are in results still, but not in site anymore:
 let difference = results.filter(x => site.indexOf(x) === -1);
 
//For each of those, change icon back and remove from results. 
 if (difference.length > 0) {
  for (var i = 0; i < difference.length; i++) { //might not need
    var mysite = difference[i];
...
sacnchoices = station.filter(function(inputdata) {
  return "SACN".includes(inputdata.Park);
})

misschoices = station.filter(function(inputdata) {
  return "MISS".includes(inputdata.Park);
})

isrochoices = station.filter(function(inputdata) {
  return "ISRO".includes(inputdata.Park);
})

voyachoices = station.filter(function(inputdata) {
  return "VOYA".includes(inputdata.Park);
})

slbechoices = station.filter(function(inputdata) {
  return "SLBE".includes(inputdata.Park);
})

induchoices = station.filter(function(inputdata) {
  return "INDU".includes(inputdata.Park);
})

apischoices = station.filter(function(inputdata) {
  return "APIS".includes(inputdata.Park);
})

pirochoices = station.filter(function(inputdata) {
  return "PIRO".includes(inputdata.Park);
})

///////////////////////////////////////////


viewof sacnIn = Inputs.checkbox(
  sacnchoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof missIn = Inputs.checkbox(
  misschoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof isroIn = Inputs.checkbox(
  isrochoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof voyaIn = Inputs.checkbox(
  voyachoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof slbeIn = Inputs.checkbox(
  slbechoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof induIn = Inputs.checkbox(
  induchoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof apisIn = Inputs.checkbox(
  apischoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)

viewof piroIn = Inputs.checkbox(
 pirochoices.map(d => d.Name), 
  {label: "Site:", unique: true, sort: true, multiple: true}
)
...
```{ojs}
viewof sacnParks = conditionalShow(await viewof sacnIn, {
...
viewof missParks = conditionalShow(await viewof missIn, {
...
viewof isroParks = conditionalShow(await viewof isroIn, {
...
viewof voyaParks = conditionalShow(await viewof voyaIn, {
...
viewof slbeParks = conditionalShow(await viewof slbeIn, {
...
viewof induParks = conditionalShow(await viewof induIn, {
...
viewof apisParks = conditionalShow(await viewof apisIn, {
...
viewof piroParks = conditionalShow(await viewof piroIn, {
...
viewof yearIn = interval([2005, 2023], {
  step: 1,
  value: [2005, 2023],
  label: 'Years',
})

viewof monthIn = interval([1, 12], {
  step: 1,
  value: [1, 12],
  label: 'Months',
})

viewof depthIn = interval([0, 50], {
  step: 0.5,
  value: [0, 50],
  label: 'Depth (m)'
})
